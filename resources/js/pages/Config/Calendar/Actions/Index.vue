...
function openEdit(row: Row) {
    editing.value = row;
    form.value = { name: row.name ?? '', active: !!row.active };
    isOpen.value = true;
}

function submit() {
    if (editing.value) {
        router.put(`/config/calendar/actions/${editing.value.id}`, form.value, {
            preserveScroll: true,
        });
    } else {
        router.post(`/config/calendar/actions`, form.value, {
            preserveScroll: true,
        });
    }
    isOpen.value = false;
...
function destroy(row: Row) {
    if (!confirm(`Remover a ação "${row.name}"?`)) return;
    router.delete(`/config/calendar/actions/${row.id}`, {
        preserveScroll: true,
    });
}
...
function applySearch() {
    router.get(
        '/config/calendar/actions',
        { search: q.value || undefined },
        { preserveState: true, replace: true },
    );
}

const rows = computed(() => props.actions?.data ?? []);
    ...
    <div class="max-w-6xl space-y-6 p-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold">
                Configurações · Calendário · Ações
            </h1>
        ...
        <Card>
            ...
            <CardContent>
                ...
                <div
                    v-if="props.actions?.links?.length"
                    class="mt-4 flex flex-wrap gap-2"
                >
                    <Link
                        v-for="l in props.actions.links"
                        :key="l.url ?? l.label"
                        :href="l.url ?? '#'"
                        preserve-scroll
                        :class="[
