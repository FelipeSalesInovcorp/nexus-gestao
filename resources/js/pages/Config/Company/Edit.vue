        ...
        <form class="space-y-6" @submit.prevent="submit">
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div class="space-y-2">
              ...
              <Input v-model="form.tax_number" placeholder="NIF" />
              <p v-if="form.errors.tax_number" class="text-sm text-red-600">{{ form.errors.tax_number }}</p>
            </div>

            <div class="space-y-2 md:col-span-2">
              <Label>Morada</Label>
              <Input v-model="form.address" placeholder="Morada" />
              <p v-if="form.errors.address" class="text-sm text-red-600">{{ form.errors.address }}</p>
            </div>

            <div class="space-y-2">
              <Label>Código Postal</Label>
              <Input v-model="form.postal_code" placeholder="XXXX-XXX" />
              <p v-if="form.errors.postal_code" class="text-sm text-red-600">{{ form.errors.postal_code }}</p>
            </div>

            <div class="space-y-2">
              <Label>Localidade</Label>
              <Input v-model="form.locality" placeholder="Cidade" />
              <p v-if="form.errors.locality" class="text-sm text-red-600">{{ form.errors.locality }}</p>
            </div>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div v-if="currentLogo" class="flex items-center gap-4">
              <img :src="currentLogo" alt="Logo" class="h-14 w-14 rounded-md border object-contain" />
              <div class="flex items-center gap-2">
                <Checkbox id="remove_logo" v-model:checked="form.remove_logo" />
                <Label for="remove_logo">Remover logotipo</Label>
              </div>
            </div>
