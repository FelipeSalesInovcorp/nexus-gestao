...
function convertToOrder() {
    if (!confirm('Converter esta proposta em Encomenda (Rascunho)?')) return;
    ...
    router.post(
        ...
        {},
        {
            preserveScroll: true,
            onSuccess: () => {
                // para demo: ir logo para as encomendas
                router.visit('/orders');
            },
            onError: () => {
                alert(
                    'Erro ao converter. Abre F12 → Network/Console para ver detalhes.',
                );
            },
        },
    );
            ...
            <CardContent>
                <form class="space-y-4" @submit.prevent="submit">
                    ...
                    <div class="flex justify-end gap-6 text-sm">
                        <div>
                            Subtotal: <b>{{ totals.subtotal }}</b>
                        </div>
                        <div>
                            IVA: <b>{{ totals.taxTotal }}</b>
                        </div>
                        <div>
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        >
                        <a
                            :href="`/proposals/${props.proposal.id}/pdf`"
                            target="_blank"
              ...
              if (confirm('Converter esta proposta em Encomenda (Rascunho)?')) {
                form.post(`/proposals/${props.proposal.id}/convert-to-order`)
              }
            }">
